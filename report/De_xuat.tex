...
Chương này trình bày phần thực nghiệm độc lập của nhóm theo ba tầng tách biệt: \textbf{Bàn luận về công bố của tác giả}, \textbf{tái hiện thí nghiệm} và \textbf{mở rộng/cải tiến trên cùng tập kiểm tra}. Toàn bộ mã nguồn, tệp cấu hình và kết quả thực nghiệm của nghiên cứu
này được công khai tại GitHub của nhóm:
\refcite{39}{39}.

\section{Bàn luận về công bố của tác giả}

\subsection{Ý nghĩa của các bảng so sánh trong bài báo}

Với Dataset 1, Bảng~\ref{tab:compare1} của tác giả so sánh ba nhóm kết quả: kết quả tham chiếu từ công trình \refcite{21}{21}, phương pháp của tác giả khi chưa balancing và phương pháp của tác giả sau balancing. Tuy nhiên, support trong bảng thay đổi rõ rệt: hai cột đầu sử dụng tổng support 1.022, trong khi cột sau balancing có tổng support 1.944 với hai lớp gần cân bằng. Do đó, các chỉ số Precision, Recall và F1 sau balancing không được tính trên cùng phân bố đánh giá với hai cột trước đó.

Với Dataset 2, Bảng~\ref{tab:d2pca} của tác giả được trình bày như một so sánh giữa mô hình không dùng PCA và mô hình có dùng PCA. Điểm cần lưu ý là support của hai cấu hình không giống nhau: cấu hình without PCA có tổng support 274.354, còn cấu hình with PCA có tổng support 21.218. Về nguyên tắc, PCA chỉ làm giảm số chiều đặc trưng, không tự làm giảm số quan sát. Vì vậy, sự thay đổi support phải đến từ một bước lấy mẫu hoặc một tập đánh giá khác chưa được mô tả đủ chi tiết trong bài báo.

\begin{table}[H]
\centering
\small
\caption{Kết quả của tác giả}
...
\midrule
Bảng~\ref{tab:compare1} - D1 & \refcite{21}{21} & 960 & 62 & 1.022 & 20\% của Dataset 1, chưa cân bằng test \\
Bảng~\ref{tab:compare1} - D1 & (a) & 960 & 62 & 1.022 & Cùng support với \refcite{21}{21}, chưa balancing \\
Bảng~\ref{tab:compare1} - D1 & (b) & 976 & 968 & 1.944 & Không cùng support 20\% mẫu khi balancing \\
\midrule
Bảng~\ref{tab:d2pca} - D2 & Without PCA & 136.758 & 137.596 & 274.354 & Support gần cân bằng do có bước sampling trước đó \\
Bảng~\ref{tab:d2pca} - D2 & With PCA & 10.613 & 10.605 & 21.218 & Support thay đổi mạnh; PCA không giải thích được việc giảm số dòng \\
\bottomrule
\end{tabular}}
\end{table}
...
\section{Phương thức tái hiện của nhóm}
...
Bảng~\ref{tab:our_balancing_counts} trình bày rõ số mẫu lớp 0 và lớp 1 trong từng giai đoạn. Điểm quan trọng là \textbf{support của test không thay đổi sau balancing}. Đây là khác biệt lớn so với cách support xuất hiện trong Bảng~\ref{tab:compare1} và Bảng~\ref{tab:d2pca} của tác giả.

\begin{table}[H]
\centering
\small
\caption{Số mẫu trong phương thức của nhóm trước và sau balancing}
...
Để nhất quán với cách tác giả trình bày Bảng~\ref{tab:compare1} và Bảng~\ref{tab:d2pca}, nhóm bổ sung classification report cho pipeline tái hiện cuối cùng. Các bảng dưới đây trình bày Precision, Recall, F1-score và Support theo từng lớp, kèm Accuracy, Macro average và Weighted average.
...
\subsubsection{Đối chiếu với \refcite{21}{21} và hai cấu hình trong Bảng~\ref{tab:compare1}}

Để làm rõ ý nghĩa của kết quả tái hiện, Bảng~\ref{tab:d1_table5_benchmark} đặt kết quả \refcite{21}{21}, hai cấu hình (a), (b) của tác giả, pipeline tái hiện và phương án cải tiến của nhóm cạnh nhau. Cột support được giữ trong bảng vì đây là thông tin quyết định việc các metric có thể được so sánh trực tiếp hay chỉ mang tính tham khảo.

\begin{table}[H]
\centering
\scriptsize
\caption{Dataset 1: đối chiếu Bảng~\ref{tab:compare1} với kết quả tái hiện và cải tiến của nhóm}
\label{tab:d1_table5_benchmark}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrrp{4.8cm}}
\toprule
\textbf{Phương án} & \textbf{Accuracy} & \textbf{Precision (1)} & \textbf{Recall (1)} & \textbf{F1 (1)} & \textbf{Support} & \textbf{Diễn giải}\\
\midrule
\refcite{21}{21} & 0,85 & 0,21 & 0,52 & 0,29 & 960/62 & Mốc tham chiếu trên test mất cân bằng, tổng 1.022 mẫu\\
Tác giả (a) & 0,92 & 0,24 & 0,15 & 0,18 & 960/62 & Accuracy/Precision cao hơn \refcite{21}{21} nhưng Recall và F1 giảm mạnh\\
Tác giả (b) & 0,95 & 0,93 & 0,96 & 0,95 & 976/968 & Support đổi sang gần 50:50; không so trực tiếp với các cột test mất cân bằng\\
Nhóm -- tái hiện & 0,7818 & 0,1554 & \textbf{0,7800} & 0,2591 & 972/50 & Recall cao hơn \refcite{21}{21} và (a), đổi lại Accuracy/Precision thấp hơn\\
Nhóm -- cải tiến & 0,8434 & 0,1944 & \textbf{0,7000} & \textbf{0,3043} & 972/50 & F1 cao hơn \refcite{21}{21} và (a), Recall cao hơn rõ rệt; Accuracy gần \refcite{21}{21}\\
\bottomrule
\end{tabular}}
\end{table}

So với \refcite{21}{21}, pipeline tái hiện của nhóm chưa cao hơn ở mọi chỉ số: Accuracy giảm từ 0,85 xuống 0,7818, Precision giảm từ 0,21 xuống 0,1554 và F1 giảm nhẹ từ 0,29 xuống 0,2591; ngược lại Recall tăng đáng kể từ 0,52 lên 0,78. Khi chuyển sang phương án cải tiến, F1 đạt 0,3043, cao hơn mức 0,29 của \refcite{21}{21}, đồng thời Recall vẫn đạt 0,70, cao hơn 0,52. Accuracy 0,8434 gần mức 0,85 của \refcite{21}{21} và Precision 0,1944 thấp hơn nhẹ mức 0,21. Vì vậy, kết quả cải tiến có thể được diễn giải là \textbf{cân bằng phát hiện lớp stroke tốt hơn \refcite{21}{21} về Recall và F1, với một đánh đổi nhỏ ở Accuracy và Precision}; tuy nhiên đây vẫn là đối chiếu tham khảo vì exact test samples không hoàn toàn giống nhau.
...
\section{Tổng hợp: tái hiện và cải tiến trên cùng test set}

Bảng~\ref{tab:final_summary} chỉ tổng hợp các kết quả do nhóm thực nghiệm trên \textbf{cùng test split giữ nguyên phân bố lớp}. Kết quả công bố của tác giả không được đưa vào bảng này vì support và phương thức đánh giá không tương đương. Nhờ đó, mọi kết luận tăng/giảm dưới đây đều là so sánh trực tiếp và có cùng mẫu đánh giá.

\begin{table}[H]
\centering
\scriptsize
\caption{Tổng hợp baseline tái hiện và phương án cải tiến của nhóm trên cùng test set}
\label{tab:final_summary}
\resizebox{\linewidth}{!}{%
\begin{tabular}{llrrp{7.0cm}}
\toprule
\textbf{Dữ liệu} & \textbf{Metric} & \textbf{Tái hiện} & \textbf{Cải tiến} & \textbf{Diễn giải}\\
\midrule
